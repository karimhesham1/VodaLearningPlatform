...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query("SELECT p FROM Post p JOIN p.tags t WHERE t.tag IN :tagNames AND p.title LIKE %:title%")
    List<Post> findByTitleAndTags(String title, Set<String> tags);
    List<Post> findByTitle(String title);
    @Query("SELECT p FROM Post p JOIN p.tags t WHERE t.tag IN :tagNames")
